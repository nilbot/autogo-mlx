...
def main() -> None:
    parser = argparse.ArgumentParser(description="Mugo Phase 8a Self-play Game Collector")
    parser.add_argument("--checkpoint", type=str, required=True, help="Path to MLX model weights (.safetensors)")
    parser.add_argument("--num-games", type=int, default=200, help="Number of games to collect")
    parser.add_argument("--n-simulations", type=int, default=64, help="Simulations per move")
    parser.add_argument("--save-dir", type=str, default="experiments/000_smoke/selfplay", help="Where to save games")
    ...
    print(f"\nCollection completed in {duration:.1f} seconds ({duration/args.num_games:.1f}s/game average).", flush=True)
