...
def main():
    print("🔬 RUNNING MCTS DIAGNOSTICS FOR STEP 2...")
    
    # Reconstruct board after 2 moves
    board = GoBoard(9)
    board.play(8, 8) # Black
    board.play(2, 8) # White
    ...
    def evaluator_cb(state: GoBoard):
        ...
        if callback_count <= 2:
            ...
            sorted_pol = sorted(policy_nn.items(), key=lambda x: x[1], reverse=True)[:5]
            print("  Top priors:")
