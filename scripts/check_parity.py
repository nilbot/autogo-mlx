...
# Ensure we import from both autogo_mlx and third_party/autogo
...
from autogo_mlx.model import SizeInvariantGoResNet as MLXGoResNet
    ...
    def copy_layer(mlx_m, pt_m, name_path: str = ""):
        ...
        if hasattr(pt_m, "weight") and hasattr(mlx_m, "weight"):
            ...
            if isinstance(pt_m, torch.nn.Conv2d):
                ...
                print(
                    f"Copied Conv weight: {name_path}.weight (shape: {pt_m.weight.shape})"
                )
            else:
            ...
            for child_name, pt_child in pt_m.named_children():
                if hasattr(mlx_m, child_name):
                    ...
                    copy_layer(
                        mlx_child,
                        pt_child,
                        f"{name_path}.{child_name}" if name_path else child_name,
                    )
                else:
                    print(
                        f"WARNING: Submodule {child_name} not found in MLX model!",
                        file=sys.stderr,
                    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--checkpoint", type=str, help="Optional MLX checkpoint .safetensors path"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="Seed for random initialization / input generation",
    )
    ...
    print(
        "Masked out last row and column of the last batch element to verify masking parity."
    )
    ...
    print(
        f"Policy Logits Diff: Max = {max_policy_err:.4e}, Mean = {mean_policy_err:.4e}"
    )
    ...
    tol = 1e-3
    assert max_policy_err <= tol, (
        f"Policy logits mismatch exceeds tolerance: {max_policy_err:.4e} > {tol:.4e}"
    )
    assert max_value_err <= tol, (
        f"Value logits mismatch exceeds tolerance: {max_value_err:.4e} > {tol:.4e}"
    )
