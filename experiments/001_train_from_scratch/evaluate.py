...
from mugo.agents.random import RandomAgent
from mugo.batched_inference import BatchedMLXEvaluator
from mugo.gameplay import play_game
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Mugo Phase 10 Model Evaluation against Random")
    ...
    parser.add_argument("--seed", type=int, default=1000, help="Base random seed (distinct from collection seed)")
    ...
    print(f"Starting evaluation match: Model {checkpoint_path} vs RandomAgent", flush=True)
    print(f"Config: games={args.num_games}, simulations={args.n_simulations}, workers={args.num_workers}, seed={args.seed}", flush=True)
    ...
    print(f"Random Wins: {random_wins} / {args.num_games} ({100 - win_rate:.2f}%)", flush=True)
    print("==========================================================", flush=True)
    
    if win_rate >= 80.0:
        print(f"SUCCESS: Model met target win rate of >= 80% (achieved {win_rate:.2f}%)!", flush=True)
        sys.exit(0)
    else:
        print(f"WARNING: Model did not meet target win rate of >= 80% (achieved {win_rate:.2f}%)", flush=True)
