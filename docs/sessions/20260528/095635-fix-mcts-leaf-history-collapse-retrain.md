...
- `[x]` Investigate and resolve Attempt 2 collapse (7% win rate, empty virtual leaf history shift)
  - `[x]` Implement fast backtracking path finder `find_virtual_path` in `gameplay.py`
  - `[x]` Integrate dynamic virtual leaf history tracking in `nn_mcts.py`
  - `[x]` Exclude PASS moves from backtracking to prevent search state explosion
  - `[x]` Verify full pytest suite (19/19 green)
...
2. **Deterministic Backtracking**: Successfully tested backtracking paths up to depth 5 in under **4 milliseconds** with 100% success.
3. **No MCTS Hangs**: Excluded PASS moves, completely resolving the MCTS state explosion hang.
