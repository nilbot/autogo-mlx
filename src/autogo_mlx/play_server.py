...
class MugoGame:
    def __init__(
        ...
        self.move_history: List[
            Tuple[int, int] | None
        ] = []  # List of coordinates (row, col) or None for pass
    ...
    def _detect_channels(self, checkpoint_path: Path) -> int:
        try:
            ...
            if "input_conv.weight" in weights:
                ...
                print(
                    f"[{self.game_id}] Auto-detected channel size = {channels} from {checkpoint_path.name}"
                )
                return channels
        except Exception as e:
            print(
                f"[{self.game_id}] Warning: failed to auto-detect channels: {e}. Falling back to 8."
            )
        return 8
    ...
    def to_state_dict(self, message: str = "") -> dict:
        ...
        return {
            ...
            "last_move_was_pass": len(self.move_history) > 0
            and self.move_history[-1] is None,
        }
    ...
    def run_mcts_analysis(self) -> dict:
        ...
        if self.in_channels == 18:
            ...
            history_boards = [
                self.board_states[i] for i in range(len(self.board_states) - 2, -1, -1)
            ]
        ...
        def mcts_evaluator_cb(state: GoBoard) -> Tuple[Dict[int, float], float]:
            ...
            policy_cpp = {
                (a if a < pass_index else PASS_ACTION): p for a, p in policy_nn.items()
            }
        ...
        return {"root_value": root_value, "moves": moves_analysis}
...
async def new_game(req: NewGameRequest) -> dict:
    ...
    if not checkpoint_path:
        raise HTTPException(
            status_code=404, detail=f"Model checkpoint '{req.model_name}' not found."
        )
    ...
    if human_color == 2:  # WHITE
        ...
        if bot_analysis["moves"]:
            best_move = max(
                bot_analysis["moves"], key=lambda m: (m["visits"], m["prior"])
            )
...
async def serve_index():
    ...
    return HTMLResponse(
        "<h2>Web frontend directory not found. Please verify folder setup.</h2>"
    )
