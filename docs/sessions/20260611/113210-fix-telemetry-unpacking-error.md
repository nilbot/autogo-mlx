# Session Handoff: Fix Telemetry Unpacking Error

- **Date**: 2026-06-11 11:32:10
- **Conversation ID**: `78f9c0ac-be31-429b-981e-a320ee9d6e72`

## 📌 Project Overview & Handoff Summary

### Original User Request
> There's error reported in training in progress: 🧪 Live Self-Play Offline Validation: ⚠️ Validation processing skipped due to error: too many values to unpack (expected 3)
...
- [/] Iteration 10 -> 20 (Phase 2: Transition to feat/phase2-regularized-value-training starting at Iteration 10 self-play, enabling replay symlinking, dense spatial ownership head, warm-up, and opponent pooling)
  ...
  - [/] Generate Iteration 11 self-play games (league opponent: iter9)
- [/] Track telemetry (FPR, resignation rate, double-pass ending frequency) at each iteration (Iter 11 running)
...
### Current Progress (Task-1902)
* The active background training loop `task-1902` is running cleanly through Iteration 20.
* Currently generating **Iteration 11 self-play games** (completed ~1840/3000 high-sim games against historical opponent `iter9.safetensors`).
* Since the loop executes the telemetry script in a fresh subprocess at the end of each iteration, the upcoming validation check for `iter12.safetensors` will automatically load the updated script from disk and execute successfully.
